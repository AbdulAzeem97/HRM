...
class SalaryLoanController extends Controller {
	...
	public function store(Request $request, Employee $employee)
	{
		...
		if (auth()->user()->can('store-details-employee'))
		{
			...
			$data['loan_time'] = (int) $request->loan_time;
			$data['time_remaining'] = (int) $request->loan_time;
			$data['amount_remaining'] = (float) $request->loan_amount;

			$data ['monthly_payable'] = round(($data['loan_amount'] / $data['loan_time']), 2);
	...
	public function update(Request $request)
	{
		if (auth()->user()->can('modify-details-employee'))
		{
			...
			$data['loan_type_id'] = $request->loan_type_id;
			$data['loan_time'] = (int) $request->loan_time;
			$data['loan_amount'] = (float) $loan->loan_amount;

			$paid_month = (int) $loan->loan_time - (int) $loan->time_remaining;

			$data['time_remaining'] = $data['loan_time'] - $paid_month ;
			$data ['monthly_payable'] = round(($data['loan_amount'] / $data['time_remaining']), 2);
            // $data ['monthly_payable'] = bcdiv(($data['loan_amount'] / $data['loan_time']), 1, 2);
