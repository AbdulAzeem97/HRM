...
-- Verification: Show attendance with different shifts in the same month
SELECT
    'Mid-Month Shift Change Test Results' as test_results,
    '' as details
UNION ALL
SELECT
    'Employee 65 - May 2025 Attendance Summary', ''
UNION ALL
SELECT '', ''
UNION ALL
SELECT
    ...
    CONCAT(
        ...
        TIME(CASE
            ...
            ELSE os.sunday_out
        END), ') | ',
        'In: ', TIME(clock_in), ' Out: ', TIME(clock_out)
    )
FROM attendances a
...
SELECT
    '' as separator,
    'Shift Change History' as history
UNION ALL
SELECT
    CONCAT('Effective: ', effective_date),
    CONCAT('Changed from ', os1.shift_name, ' to ', os2.shift_name, ' - Reason: ', reason)
