<?php
/**
 * Execute the attendance upload directly
 */
...
echo "🚀 STARTING ATTENDANCE DATA UPLOAD\n";
echo "==================================\n\n";

// Use direct database connection since Laravel might not load properly
$host = 'localhost';
...
try {
    ...
    // Prepare statements
    $insertSql = "INSERT INTO attendances (employee_id, attendance_date, clock_in, clock_out, clock_in_ip, clock_out_ip, clock_in_out, time_late, early_leaving, overtime, total_work, total_rest, attendance_status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
    ...
    foreach ($attendanceData as $record) {
        ...
        try {
            ...
            if ($checkStmt->fetchColumn() > 0) {
                echo "⚠️  Skipped: Attendance exists for staff $staffId on $attendanceDate\n";
                $skippedCount++;
                continue;
            }
            ...
            $successCount++;
            echo "✅ Added: Staff $staffId on $attendanceDate ($clockInFormatted - $clockOutFormatted)\n";

        } catch (Exception $e) {
            echo "❌ Error processing staff $staffId on $dateStr: " . $e->getMessage() . "\n";
            $errorCount++;
        }
    }
    ...
    if ($successCount > 0) {
        echo "🎉 SUCCESS! $successCount attendance records have been uploaded to your database!\n";
        echo "💡 The attendance data for staff IDs " . implode(', ', array_keys($staffToEmployeeId)) . " is now available in your system.\n";
        echo "📊 You can view the attendance records in your attendance management interface.\n";
    } else {
        echo "⚠️  No new records were added. This could be because:\n";
        echo "   • The records already exist in the database\n";
        echo "   • The staff IDs don't match existing employees\n";
        echo "   • There were data format issues\n";
    }
