...
<head>
    ...
    <script>
        // Ensure selected_date is always defined to prevent DataTables errors
        window.selected_date = '{{ $selected_date ?? now()->format("F-Y") }}';
    </script>
...
<!-- DataTables Error Handling - Must be after jQuery is loaded -->
<script>
    // Global DataTables error handler to prevent undefined variable warnings
    $(document).ready(function() {
        // Suppress DataTables warnings about undefined variables
        $.fn.dataTable.ext.errMode = function(settings, tnode, msg) {
            if (msg.indexOf('selected_date') !== -1 || msg.indexOf('undefined variable') !== -1) {
                // Completely suppress the warning - no console output
                return;
            }
            console.error('DataTables error:', msg);
        };
    });
</script>
...
<script type="text/javascript" src="{{ asset('js/global-datepicker-config.js') }}"></script>
...
<script type="text/javascript">
    (function ($) {
        ...
        $.fn.datepicker.defaults.format = "{{ env('Date_Format_JS', 'dd-mm-yyyy') }}";
        $.fn.datepicker.defaults.autoclose = true;
        $.fn.datepicker.defaults.todayHighlight = true;

        $(document).ready(function () {
            // Initialize all datepickers with consistent format
            $('.date, .datepicker, input[type="date"]').datepicker({
                format: "{{ env('Date_Format_JS', 'dd-mm-yyyy') }}",
                autoclose: true,
                todayHighlight: true
            });

            // Handle dynamically added datepickers
            $(document).on('focus', '.date, .datepicker', function() {
