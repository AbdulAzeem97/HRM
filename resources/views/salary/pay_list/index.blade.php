    ...
    (function($) {
        ...
        $(document).ready(function () {
            ...
            function fill_datatable(filter_company = '', filter_department = '',filter_month_year = '') {
                // Ensure we always have a month_year value
                if (!filter_month_year) {
                    filter_month_year = $('#month_year').val() || new Date().toLocaleDateString('en-US', {month: 'long', year: 'numeric'});
                }

                // Additional safety check - ensure we have a valid month_year
                if (!filter_month_year || filter_month_year === '' || filter_month_year === 'undefined') {
                    filter_month_year = new Date().toLocaleDateString('en-US', {month: 'long', year: 'numeric'});
                }

                $('#details_month_year').html(filter_month_year);
                let table_table = $('#pay_list-table').DataTable({
                    ...
                    ajax: {
                        ...
                        data: function(d) {
                            ...
                            d.filter_department = filter_department;
                            d.filter_month_year = filter_month_year;
                            d._token = "{{ csrf_token()}}";
                            d._timestamp = new Date().getTime(); // Cache busting
                        },
                        cache: false, // Disable caching
                        error: function(xhr, error, thrown) {
                            console.log('DataTables AJAX error handled:', error);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.success) {
                        ...
                        $('#pay_list-table').DataTable().ajax.reload(null, false); // Force reload without resetting pagination
        ...
        $(document).on('click', '.delete', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#pay_list-table').DataTable().ajax.reload(null, false);
